...
router.get("/search", async (req, res) => {
  try {
    ...
    let queryConditions = [];
    if (albumTitle)
      queryConditions.push({ albumTitle: new RegExp(albumTitle, "i") });
    if (artistName)
      queryConditions.push({ artistName: new RegExp(artistName, "i") });
    if (bandMember)
      queryConditions.push({
        bandMembers: {
          $elemMatch: { memberName: new RegExp(bandMember, "i") },
        },
      });
    if (trackTitle)
      queryConditions.push({
        "tracks.trackTitle": new RegExp(trackTitle, "i"),
      });
    if (releaseYear) {
